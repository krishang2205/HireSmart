...
export const PasswordField = forwardRef<HTMLInputElement, PasswordFieldProps>(( { strength, showStrength, dense, ...props }, ref) => {
  ...
  return (
    <div className={dense ? 'space-y-1' : 'space-y-1.5'}>
      <div className="relative">
        ...
        <button type="button" aria-label={visible ? 'Hide password' : 'Show password'} onClick={() => setVisible(v=>!v)} className={`absolute right-3 ${dense ? 'top-2.5' : 'top-3'} text-[11px] font-medium text-slate-500 hover:text-slate-700 focus:outline-none`}>
          {visible ? 'Hide' : 'Show'}
        </button>
      </div>
      {showStrength && typeof strength === 'number' && strength >=0 && (
        <div className={dense ? 'space-y-0.5' : 'space-y-1'}>
          <div className={`w-full rounded bg-slate-200 overflow-hidden ${dense ? 'h-1' : 'h-1.5'}`}> 
            <div className={`h-full transition-all duration-500 ${colors[strength]}`} style={{ width: `${(strength+1)*20}%` }} />
          </div>
          <p className="text-[10.5px] text-slate-500">Strength: <span className="font-medium text-slate-700">{['Very weak','Weak','Fair','Good','Strong'][strength]}</span></p>
        </div>
      )}
    </div>
  );
